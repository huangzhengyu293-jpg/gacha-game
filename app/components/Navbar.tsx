...
import { KeyDropMainHeader } from './KeyDropMainHeader';
...
export default function Navbar() {
  ...
  /** 与桌面主导航 min-[910px] 一致：中屏弹窗仅在 sm～909px */
  ...
  return (
    <div className="relative z-20 flex w-full flex-col">
      ...
      <KeyDropMainHeader
        ...
        walletBalanceUsd={Number(beanDisplay)}
        onOpenWalletDeposit={() => setShowWalletModal(true)}
        userAvatarUrl={user?.userInfo?.avatar ?? null}
        userDisplayName={user?.userInfo?.name || user?.userInfo?.email || ''}
        profileHref="/account"
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className={`${keyDropPoppins.className} flex min-[910px]:hidden flex-col fixed left-0 right-0 top-[50px] bottom-0 bg-[#1B1B22] md:top-[70px] min-[2560px]:top-[90px]`}>
          {isAuthenticated ? (
          ...
          <div className="flex flex-col px-6">
            {MAIN_NAV_GAMES.map((item) => (
              <Link
                key={item.key}
                href={item.href}
                className="menu-item flex w-full items-center gap-2 rounded-lg py-3"
                onClick={() => setIsMenuOpen(false)}
              >
                <div className="size-5 shrink-0 text-[#7A8084]">
                  <MainNavGameIcon name={item.key} className="h-5 w-5" />
                </div>
