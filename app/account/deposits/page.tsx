...
export default function DepositsPage() {
  ...
  const { data } = useRechargeLog({ enabled: isAuthenticated });
      ...
      <div className="flex flex-col lg:flex-row items-start gap-0 lg:gap-10">
        ...
        <div className="hidden lg:flex flex-col gap-4 w-[220px] flex-none">
          <div className="flex flex-col gap-3 items-stretch w-full">
            ...
            <Link href="/account/deposits" className="inline-flex items-center gap-2 whitespace-nowrap rounded-md transition-colors interactive-focus relative font-bold select-none h-10 px-6 justify-start text-md acct-menu-item--active">
        ...
        <div className="flex flex-col items-start w-full lg:flex-1 min-w-0 gap-2">
          <div className="flex justify-between items-center self-stretch pb-1 pt-4 lg:pt-0 min-w-0">
            <AccountMobileMenu />
            <h1 className="text-2xl font-bold hidden lg:block" style={{ color: '#FFFFFF' }}>{t('accountDepositsTitle')}</h1>
          </div>
          {records.length === 0 ? (
            <div className="flex flex-col gap-3 items-center justify-center py-12 self-stretch">
              <span className="font-semibold" style={{ color: '#FFFFFF' }}>{t('noDeposits')}</span>
            </div>
          ) : (
            <div className="self-stretch space-y-6 z-10">
              {records.map((item: any, index: number) => {
                ...
                const titleLeft = `${formattedAmount} ${channel}`;
                  ...
                  >
                    <div className="flex justify-between items-center pb-4" style={{ color: '#FFFFFF' }}>
                      <div className="flex gap-3 items-center">
                        <h2 className="whitespace-nowrap" style={{ color: '#FFFFFF' }}>{titleLeft}</h2>
                      </div>
