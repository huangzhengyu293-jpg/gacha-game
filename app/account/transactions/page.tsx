...
export default function TransactionsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["accountTransactionHistory"],
    queryFn: () => api.getLuckyLog(),
    staleTime: 30_000,
      ...
      <div className="flex flex-col lg:flex-row items-start gap-0 lg:gap-10">
        <div className="hidden lg:flex flex-col gap-4 w-[220px] flex-none">
          <div className="flex flex-col gap-3 items-stretch w-full">
            ...
            <Link href="/account/transactions" className="inline-flex items-center gap-2 whitespace-nowrap rounded-md transition-colors interactive-focus relative font-bold select-none h-10 px-6 justify-start text-md acct-menu-item--active">
          ...
          {isLoading ? (
            ...
            <div className="self-stretch space-y-6 z-10">
              {records.map((item: any, idx: number) => {
                ...
                const title = steam?.name || item?.name || item?.item_name || `记录 ${idx + 1}`;
                const cover = steam?.cover;
                const createdAt = item?.created_at || item?.time || item?.updated_at || "";
                const winPercentRaw = item?.user_percent;
                const winPercent =
                  winPercentRaw === null || winPercentRaw === undefined
                    ? "--"
                    : `${Number(winPercentRaw) * 100 % 1 === 0 ? Number(winPercentRaw) * 100 : (Number(winPercentRaw) * 100).toFixed(2)}%`;
                const betAmount = "--";
                const payRaw = item?.money;
                const payLabel =
                  payRaw === null || payRaw === undefined ? "-" : `$${Number(payRaw || 0).toFixed(2)}`;
                const brand = "--";
                const productLabel = steam?.name || title;
                    ...
                    <div className="flex justify-between items-center text-white">
                      <div className="flex gap-10 max-w-[100%] lg:max-w-[calc(100%-150px)]">
                        <div className="flex-none flex flex-col items-start gap-2">
                          <span className="font-semibold">{t('winChanceLabel')}:</span>
                          <span className="font-semibold">{t('betAmountLabel')}:</span>
                          <span className="font-semibold">{t('payoutLabel')}:</span>
                        </div>
                        <div className="flex flex-col items-start overflow-hidden max-w-full gap-2">
                          <span className="truncate max-w-full">{winPercent}</span>
                          <span className="truncate max-w-full">{betAmount}</span>
                          <span className="truncate max-w-full">{payLabel}</span>
                        </div>
                      </div>
